...
class Cover(object):
    '''Voverage vector (wrapper) includes "first_span", so that Cover is the signature for groups in CP.'''
...
class Signature():
    '''Signature includes a cover object, ordered list of uncovered spans and tgt_elided, so it is the only signature for each hyps'''
...
def defaultSign(sent_len):
    return Signature(defaultCover(sent_len),[(0,sent_len)],"<s>")
