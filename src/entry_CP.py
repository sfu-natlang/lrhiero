...
class Entry(object):
    ...
    __slots__ = "score", "tgt", "featVec", "tgt_elided", "depth_hier", "inf_cell", "inf_rule", "bp", "cand_score", "lm_right", "sign"
 
    def __init__(self, score, tgt, featVec, tgt_elided, sign=None, rule_depth=0, inf_cell=(), inf_rule=(None, None), bp=None, cand_score=0.0, r_lm_state=None):
        self.score = score
        self.tgt = tgt
	...
	tgt = self.tgt
	if history != "<s>":
            assert (self.tgt.startswith(history)) , "Error!!! history does not match\n history: %s\n hypothesis: %s" %( match[0], ' '.join(map(lambda x: str(x), match[1])) )
...
def getInitHyp(sent_len, initRule, history='<s>'):
        init_hyp = Entry(0, history, [0 for i in settings.opts.U_lpTup[2]], history, defaultSign(sent_len)) 
